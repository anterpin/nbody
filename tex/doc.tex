...
% ГОСТ 7.32—2017 
\usepackage[paper=a4paper, left=2cm, right=1cm, top=3cm, bottom=2cm]{geometry}
...
\end{align*}
Картинка (\ref{fig:euler verlet comparison}) показывает как интеграция \textit{Verlet} имеет гораздо меншую расходимость от изначальной функции, нежели чем методом Ейлера.
\begin{figure}[H]
  \centering
...
\begin{tikzpicture}
  \begin{axis}[xmin=0, xmax=3, ymin=0, ymax=4, xlabel=$N$, ylabel=$\operatorname{ms}$, legend pos=north east, thick]
    \draw [gray]  (0,0) -- (1,1) [point] -- (2,0.75)[point] -- (3,2) [point];
            \addplot[domain=-10:-9, color=gray, samples=3] {3};
            \addplot[color=blue, samples=100, domain=0:2*pi]({ 2*sqrt(2)*cos(deg(x)) }, { 3*sqrt(2)*sin(deg(x))});
            \legend{$\beta$, $\alpha$}
            \addplot [color=black, mark=*] coordinates {(2 * sqrt(2), 0)} node [below right=.8mm, scale=0.8]{$(2\sqrt{2}, 0)$};
          \end{axis}
